...
// 减少可用空间大小
func CallMySQLUpdateStorageUnusedCapacity(account, password, ip, port, name, userAccount string, fileSize int64) error {
	// 获取节点
	remoteAddr, err := serviceSearch.GetNode(data.MySQLServiceName)
	if err != nil {
		return err
	...
	// 调用
	var result interface{}
	err = client.Client.Call(data.MySQLServiceName+".UpdateStorageUnusedCapacity", dataObj, &result)
	// 错误处理
	if err != nil {
		return err
	}
