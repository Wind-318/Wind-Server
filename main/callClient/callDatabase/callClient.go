...
// 更新动漫资料
func CallMySQLUpdateAnimeInfo(account, password, ip, port, name, url, description, picurl, animeName string, isNew int) error {
	// 获取节点
	remoteAddr, err := serviceSearch.GetNode(data.MySQLServiceName)
	if err != nil {
		return err
...
// 插入新文件夹
func CallMySQLInsertStorageNewFolder(account, password, ip, port, name, userAccount, filename string) error {
	// 获取节点
	remoteAddr, err := serviceSearch.GetNode(data.MySQLServiceName)
	if err != nil {
		return err
	...
	// 调用
	var result interface{}
	err = client.Client.Call(data.MySQLServiceName+".InsertStorageNewFolder", dataObj, &result)
	// 错误处理
	if err != nil {
		return err
	}
...
// 插入新文件
func CallMySQLInsertStorageNewFile(account, password, ip, port, name, userAccount, filepath, filename, types, paths, smallpath string, size int64) error {
	// 获取节点
	remoteAddr, err := serviceSearch.GetNode(data.MySQLServiceName)
	if err != nil {
		return err
	...
	// 调用
	var result interface{}
	err = client.Client.Call(data.MySQLServiceName+".InsertStorageNewFile", dataObj, &result)
	// 错误处理
	if err != nil {
		return err
	}
...
// 判断文件夹是否存在
func CallMySQLSelectIsExistFolder(account, password, ip, port, name, userAccount, filename string) error {
	// 获取节点
	remoteAddr, err := serviceSearch.GetNode(data.MySQLServiceName)
	if err != nil {
		return err
	...
	// 调用
	var result interface{}
	err = client.Client.Call(data.MySQLServiceName+".SelectIsExistFolder", dataObj, &result)
	// 错误处理
	if err != nil {
		return err
	}
