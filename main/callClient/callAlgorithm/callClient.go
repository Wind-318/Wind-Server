...
// IP 访问次数加一
func CallAlgorithmAddIP(ctx *gin.Context) error {
	// 获取节点
	remoteAddr, err := serviceSearch.GetNode(data.AlgorithmServiceName)
	if err != nil {
		return err
	...
	var result interface{}
	// 调用
	err = client.Client.Call(data.AlgorithmServiceName+".AddIPAlgorithm", ctx.ClientIP(), &result)
	// 错误处理
	if err != nil {
		return err
	}
...
// 判断 IP 是否受限
func CallAlgorithmIfRestricted(ctx *gin.Context) (bool, error) {
	// 获取节点
	remoteAddr, err := serviceSearch.GetNode(data.AlgorithmServiceName)
	if err != nil {
		return false, err
	...
	var result bool
	// 调用
	err = client.Client.Call(data.AlgorithmServiceName+".IfRestrictedAlgorithm", ctx.ClientIP(), &result)
	// 错误处理
	if err != nil {
		return false, err
	}
