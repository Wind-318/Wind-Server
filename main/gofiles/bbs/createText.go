...
func CreateText(ctx *gin.Context) {
	...
	// 获取账号
	cookie, err := redis.String(callDatabase.CallRedis(config.RedisIP, config.RedisPort, "HGET", cookies, "email"))
	if err != nil {
	...
	// 加分布式锁
	if ok, err := redis.String(callDatabase.CallRedis(config.RedisIP, config.RedisPort, "SET", cookie+"CreateText", "CreateText", "EX", 10, "NX")); err != nil || ok != "OK" {
		return
	}
	...
	go func() {
		for {
			...
			// 锁存在
			if ok, err := redis.Bool(callDatabase.CallRedis(config.RedisIP, config.RedisPort, "SETNX", cookie+"CreateText", "CreateText")); err != nil || !ok {
				// 设置过期 10 秒
				callDatabase.CallRedis(config.RedisIP, config.RedisPort, "Expire", cookie+"CreateText", 10)
			} else {
				callDatabase.CallRedis(config.RedisIP, config.RedisPort, "del", cookie+"CreateText")
				return
			}
		}
	}()
	defer callDatabase.CallRedis(config.RedisIP, config.RedisPort, "del", cookie+"CreateText")
	...
	id, err := callDatabase.CallMySQLSelectUserIDByAccount(config.MySQLAccount, config.MySQLPassword, config.MySQLIP, config.MySQLPort, "user", cookie)
	if err != nil {
		result["msg"] = err.Error()
		ctx.JSON(http.StatusOK, result)
		return
	}
	name, err := callDatabase.CallMySQLSelectUserNameByAccount(config.MySQLAccount, config.MySQLPassword, config.MySQLIP, config.MySQLPort, "user", cookie)
	if err != nil {
		result["msg"] = err.Error()
		ctx.JSON(http.StatusOK, result)
		return
