...
// 更新积分数
func CallMySQLUpdateUserScore(account, password, ip, port, name, userAccount string, score int) error {
	// 获取节点
	remoteAddr, err := serviceSearch.GetNode(data.MySQLServiceName)
	if err != nil {
		return err
	...
	// 调用
	var result interface{}
	err = client.Client.Call(data.MySQLServiceName+".UpdateUserScore", dataObj, &result)
	// 错误处理
	if err != nil {
		return err
	}
